...
class CartState extends Equatable {
  ...
  CartState copyWith(
      {CartStatus? status, CartModel? cartModel, String? message}) {
    return CartState._(
  ...
  CartState.fetchCartSuccess({required CartModel? cartModel})
      : this._(cartModel: cartModel, status: CartStatus.success);
  CartState.cartError({required String? message})
      : this._(message: message, status: CartStatus.failure);
  CartState.updateCartSuccess() : this._(status: CartStatus.updateSuccess);
  CartState.updateCartError({required String? message})
      : this._(message: message, status: CartStatus.failure);
  CartState.deleteItemError({required String? message})
      : this._(message: message, status: CartStatus.failure);
