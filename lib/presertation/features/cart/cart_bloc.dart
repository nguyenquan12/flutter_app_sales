import 'package:app_sales/data/models/cart_model.dart';
import 'package:app_sales/data/repositories/cart_repository.dart';
import 'package:app_sales/presertation/features/cart/cart_event.dart';
import 'package:app_sales/presertation/features/cart/cart_state.dart';
...
class CartBloc extends Bloc<CartEventBase, CartState> {
  ...
  CartBloc(CartRepository repository) : super(CartState.initial()) {
    ...
    on<FetchListCart>((event, emit) async {
      try {
        ...
        if (dioError.response != null) {
          emit(
              CartState.cartError(message: dioError.response!.data["message"]));
        }
      } catch (e) {
    ...
    on<UpdateCart>((event, emit) async {
      try {
        emit(CartState.cartLoading());
        Response response = await _cartRepository.updateItemCart(
            event.orderId, event.foodId, event.quantity);
        if (response.statusCode == 200) {
          emit(CartState.updateCartSuccess());
        }
      } on DioException catch (dioError) {
        if (dioError.response != null) {
          emit(CartState.updateCartError(
      ...
      try {
        ...
        if (response.statusCode == 200) {
          if (response.data["data"] != null) {
            CartModel cartModel = CartModel.fromJson(response.data["data"]);
            emit(state.copyWith(
                status: CartStatus.success, cartModel: cartModel));
          } else {
        ...
        if (dioError.response != null) {
          emit(CartState.deleteItemError(
              message: dioError.response!.data["message"]));
        }
      } catch (e) {
